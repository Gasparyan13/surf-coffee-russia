...
import { mockGetAnalyticsCashFlowReport } from '@testEnv/server/handlers/analytics'
import {
  fireEvent,
  render,
  screen,
...
const createServer = () => setupServer(mockGetAnalyticsCashFlowReport())
...
describe('<CashFlowReport />', () => {
  ...
  const renderCashFlowReport = (params: ConfigFinReportCtxValueParams) =>
  ...
  describe('when render "CashFlowReport" first time', () => {
    it('should send "getAnalyticsCashFlowReport" request', async () => {
      renderCashFlowReport({})

      await waitFor(() => {
        expect(
          appStore.getState().api.queries[
            `getAnalyticsCashFlowReport({"enterpriseId":${enterpriseId},"year":${getCurrentYear()}})`
          ]?.status,
        ).toEqual('fulfilled')
      })
    })
